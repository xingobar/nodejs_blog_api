...
import { Repository, EntityRepository, getRepository, getConnection } from "typeorm";
...
export default class UserRepository extends RepositoryBase<User> {
  ...
  findByAccountWithPassword(account: string) {
    return getConnection(config.connectionName).getRepository(User).createQueryBuilder("users").addSelect("users.password").getRawOne();
  }
}
